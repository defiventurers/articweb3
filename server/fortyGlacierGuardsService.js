...
const GAME_ID = FORTY_GLACIER_GUARDS_RULESET.gameId;
const ROLES = ["aurora", "ember"];

function createFortyGlacierGuardsService(deps) {
  ...
  function normalizeRole(value) { return value === "ember" ? "ember" : "aurora"; }
  function oppositeRole(role) { return role === "aurora" ? "ember" : "aurora"; }
  function isFortyGlacierGuardsRoom(room) { return room?.gameId === GAME_ID; }
  function code() {
  ...
  function findRoom(ws, requestId, payload) {
    ...
    if (!room || !isFortyGlacierGuardsRoom(room)) {
      fail(ws, requestId, "Forty Glacier Guards room not found.");
      return null;
    }
    return room;
  }
  ...
  function broadcast(room) {
    ...
    saveRoomSafe(room);
    const packet = { type: "fgg_room_state", payload: { room: roomView(room) } };
  ...
  function joinRoom(ws, requestId, payload = {}) {
    ...
    if (room.players[wallet]) {
      ok(ws, requestId, "fgg_room_join_result", { room: roomView(room) });
      return;
    }
    ...
    const occupiedRole = players(room)[0]?.seat || "aurora";
  ...
  function getState(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "fgg_game_state_result", { room: roomView(room) });
  }
  ...
  function legalActions(ws, requestId, payload = {}) {
    ...
    const actions = room.status === "playing" && player.seat === room.gameState.currentPlayer
      ...
      : [];
    ok(ws, requestId, "fgg_legal_actions_result", { actions, room: roomView(room) });
  }
  ...
  function action(ws, requestId, payload = {}) {
    ...
    const player = room.players[wallet];
    if (!player) return fail(ws, requestId, "Join the room first.");
    if (room.status !== "playing") return fail(ws, requestId, room.status === "finished" ? "The glacier battle has ended." : "Waiting for a second player.");
    ...
    audit(room, { type: "game_action", wallet, role: player.seat, action: payload.action, turn: room.gameState.turn, chainFrom: room.gameState.chainFrom });
    if (room.gameState.winner) finalize(room);
    ok(ws, requestId, "fgg_game_action_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  async function history(ws, requestId, payload = {}) {
    ...
    if (!wallet) return;
    try {
      ok(ws, requestId, "fgg_history_result", { history: await getHistoryForWallet(wallet, GAME_ID) });
    } catch (err) {
      fail(ws, requestId, err.message || "Could not load Forty Glacier Guards history.");
    }
  }
  ...
  function myRooms(ws, requestId, payload = {}) {
    ...
    const result = [...rooms.values()]
      ...
      .map(roomView);
    ok(ws, requestId, "fgg_my_rooms_result", { rooms: result });
  }
  ...
  function recordHistory(room) {
    ...
    ps.forEach((player) => {
      ...
      saveHistoryEntry(entry).catch((err) => console.error(`[fgg-history] save failed room=${room.roomCode}: ${err.message}`));
    });
    saveRoomSafe(room);
  }

  function restoreRoom(room) {
    if (!isFortyGlacierGuardsRoom(room)) return false;
    room.rulesetVersion = room.rulesetVersion || FORTY_GLACIER_GUARDS_RULESET.rulesetVersion;
  ...
  return {
    ...
    restoreRoom,
    roomView,
    isFortyGlacierGuardsRoom
  };
}

module.exports = { createFortyGlacierGuardsService, GAME_ID, ROLES };
