...
const BOARD_SIZE = 7;
const cellId = (row, col) => `c${row}${col}`;
const CELLS = Object.freeze(Array.from({ length: BOARD_SIZE }, (_, row) => Array.from({ length: BOARD_SIZE }, (_, col) => ({ id: cellId(row, col), row, col }))).flat());
...
function buildNorthRoute() {
  ...
  return [...outer, ...ringClockwiseFromNorthEast(1, 5), ...ringClockwiseFromNorthEast(2, 4), cellId(3, 3)];
}
function rotateCell(id, quarterTurns) {
  let row = Number(id[1]); let col = Number(id[2]);
  for (let turn = 0; turn < quarterTurns; turn += 1) [row, col] = [col, BOARD_SIZE - 1 - row];
...
const sideName = (side) => side === "ember" ? "Ember Kingdom" : "Aurora Kingdom";

function createPieces(side) {
  return Array.from({ length: 4 }, (_, index) => ({ id: `${side}-${index + 1}`, side, status: "home", progress: -1 }));
}
...
function createAshtaGraceDrill() {
  ...
  state.pieces.aurora[1].status = "track"; state.pieces.aurora[1].progress = 5;
  state.pieces.aurora[2].status = "finished"; state.pieces.aurora[2].progress = FINISH_PROGRESS;
  state.pieces.aurora[3].status = "finished"; state.pieces.aurora[3].progress = FINISH_PROGRESS;
  const captureCell = ROUTES.aurora[13];
  state.pieces.ember[0].status = "track"; state.pieces.ember[0].progress = ROUTES.ember.indexOf(captureCell);
  ...
  state.bonusRolls = 1; state.awaiting = "allocate"; state.castCount = 1;
  assertStateInvariant(state);
  return state;
}
...
function getOccupantsAtSpace(state, spaceId, ignorePieceIds = []) {
  ...
  for (const side of SIDES) for (const piece of state.pieces[side] || []) {
    if (!ignored.has(piece.id) && piece.status === "track" && getPieceSpaceId(piece) === spaceId) occupants.push(piece);
  }
  return occupants;
}
...
function landingResult(state, side, targetSpace, movingPieceIds, actionType) {
  ...
  if (SAFE_SPACES.has(targetSpace)) {
    if (enemies.length && actionType !== "enter") return { legal: false };
    if (friendly.length + movingCount > 2) return { legal: false };
    return { legal: true, capturedPieceIds: [] };
  }
  if (friendly.length) return friendly.length + movingCount <= 2 ? { legal: true, capturedPieceIds: [] } : { legal: false };
  if (enemies.length) return enemies.length <= movingCount ? { legal: true, capturedPieceIds: enemies.map((piece) => piece.id).sort() } : { legal: false };
  return { legal: true, capturedPieceIds: [] };
}
function scoreCowries(faces) {
  const mouthsUp = faces.reduce((sum, face) => sum + Number(Boolean(face)), 0);
  return { mouthsUp, value: mouthsUp === 0 ? 8 : mouthsUp, grace: mouthsUp === 0 || mouthsUp === 4, splitGrace: mouthsUp === 0 };
}
...
function getLegalActions(state, side = state.currentPlayer, unitId = null) {
  ...
  for (const unit of units) {
    if (unit.enterAllowed) {
      ...
      for (const piece of state.pieces[side]) if (piece.status === "home" && landingResult(state, side, startSpace, [piece.id], "enter").legal) {
        actions.push({ type: "enter", unitId: unit.id, pieceId: piece.id, pieceIds: [piece.id], groupSize: 1, value: unit.value, targetProgress: 0, targetSpace: startSpace, captures: null, capturedPieceIds: [] });
      }
    }
    if (unit.moveAllowed) {
      const seen = new Set();
      for (const piece of state.pieces[side]) {
        ...
        for (const pieceIds of movingGroupsAtSpace(state, side, fromSpace)) {
          ...
          seen.add(key);
          const moving = pieceIds.map((id) => getPiece(state, side, id));
          const progress = moving[0].progress;
          if (!moving.every((candidate) => candidate.progress === progress)) continue;
          ...
          actions.push({ type: "move", unitId: unit.id, pieceId: pieceIds[0], pieceIds, groupSize: pieceIds.length, value: unit.value, fromSpace, targetProgress, targetSpace, finishes: targetProgress === FINISH_PROGRESS, captures: landing.capturedPieceIds[0] || null, capturedPieceIds: landing.capturedPieceIds });
        }
      }
    }
...
function validateRoll(state, faces, side = state.currentPlayer) {
  ...
  if (!Array.isArray(faces) || faces.length !== 4 || faces.some((face) => ![0, 1, false, true].includes(face))) return { valid: false, reason: "An Ashta-Kashte cast must contain four open-or-closed cowries." };
  return { valid: true };
}
...
function actionKey(action) {
  ...
  const ids = action.pieceIds?.length ? [...action.pieceIds].sort() : [action.pieceId];
  return `${action.type}:${action.unitId}:${ids.join("+")}:${action.targetProgress}`;
}
function validateAction(state, action, side = state.currentPlayer) {
  ...
  return legal ? { valid: true, action: legal } : { valid: false, reason: "That destination is blocked, protected, overfilled, or defended by a stronger pair." };
}
function consumeUnit(state, unitId) {
  state.throwPool = state.throwPool.filter((unit) => unit.id !== unitId);
  if (state.throwPool.length) return void (state.awaiting = "allocate");
  state.awaiting = "roll"; state.turn += 1;
...
function applyAction(state, action, side = state.currentPlayer) {
  ...
  if (legal.type === "pass-unit") {
    ...
    consumeUnit(next, legal.unitId); assertStateInvariant(next); return { state: next, error: null };
  }
  const pieceIds = legal.pieceIds?.length ? legal.pieceIds : [legal.pieceId];
  const moving = pieceIds.map((id) => getPiece(next, side, id));
  const fromSpace = legal.type === "enter" ? null : getPieceSpaceId(moving[0]);
  const capturedPieceIds = [...(legal.capturedPieceIds || [])];
  for (const id of capturedPieceIds) {
    const captured = getPiece(next, otherSide(side), id);
    if (captured) { captured.status = "home"; captured.progress = -1; }
  }
  if (capturedPieceIds.length) { next.captures[side] += capturedPieceIds.length; next.bonusRolls += 1; }
  ...
  next.lastAction = { type: legal.type, side, unitId: legal.unitId, pieceId: pieceIds[0], pieceIds, groupSize: pieceIds.length, value: legal.value, fromSpace, targetSpace: legal.targetSpace, targetProgress: legal.targetProgress, capturedPieceId: capturedPieceIds[0] || null, capturedPieceIds, finished: moving.every((piece) => piece.status === "finished") };
  next.history.push({ type: "action", turn: next.turn, side, action: cloneState(next.lastAction) });
  consumeUnit(next, legal.unitId);
  if (next.pieces[side].every((piece) => piece.status === "finished")) { next.winner = side; next.winReason = "all-four-finished"; next.awaiting = "finished"; next.throwPool = []; next.bonusRolls = 0; }
  assertStateInvariant(next);
  return { state: next, error: null };
}
function rollLocalCowries(state, side = state.currentPlayer) {
  let seed = Number(state.rngState || 1) >>> 0; const faces = [];
  for (let index = 0; index < 4; index += 1) { seed = (Math.imul(seed, 1664525) + 1013904223) >>> 0; faces.push((seed >>> 31) & 1); }
  const result = applyRoll(state, faces, side); if (!result.error) result.state.rngState = seed; return result;
}
function getPlayerSummary(state, side) {
  const pieces = state.pieces[side] || []; const counts = new Map();
  for (const piece of pieces) { const id = getPieceSpaceId(piece); if (id) counts.set(id, (counts.get(id) || 0) + 1); }
  return { home: pieces.filter((piece) => piece.status === "home").length, track: pieces.filter((piece) => piece.status === "track").length, finished: pieces.filter((piece) => piece.status === "finished").length, captures: Number(state.captures[side] || 0), doubles: [...counts.values()].filter((count) => count === 2).length, totalProgress: pieces.reduce((sum, piece) => sum + Math.max(0, Number(piece.progress || 0)), 0) };
}
function describeTurn(state) { if (state.winner) return resultTitle(state); return state.awaiting === "roll" ? `${sideName(state.currentPlayer)}: cast four cowries.` : `${sideName(state.currentPlayer)}: allocate or pass each stored throw.`; }
function resultTitle(state) { if (!state.winner) return "Spiral race in progress"; return state.winner === "aurora" ? "Aurora Kingdom reaches the centre" : "Ember Kingdom reaches the centre"; }
...
function assertStateInvariant(state) {
  ...
  if (pieces.length !== 8) throw new Error("Cowrie Kingdoms piece-count invariant failed.");
  ...
  for (const piece of pieces) {
    ...
    if (piece.status === "track") {
      ...
      const spaceId = getPieceSpaceId(piece); if (!spaceId) throw new Error("Track runner occupies an unknown cell.");
      const bySide = occupancy.get(spaceId) || { aurora: 0, ember: 0 }; bySide[piece.side] += 1;
      if (bySide[piece.side] > 2) throw new Error(`Friendly stack limit failed at ${spaceId}.`); occupancy.set(spaceId, bySide);
    }
  }
  for (const [spaceId, bySide] of occupancy) if (!SAFE_SPACES.has(spaceId) && bySide.aurora && bySide.ember) throw new Error(`Mixed unprotected occupancy failed at ${spaceId}.`);
...
module.exports = {
  COWRIE_KINGDOMS_RULESET, SIDES, BOARD_SIZE, CELLS, SAFE_SPACES, ROUTES, FINISH_PROGRESS, cellId,
  createCowrieKingdomsState, createAshtaGraceDrill, otherSide, routeFor, getPiece, getPieceSpaceId,
  getOccupantsAtSpace, scoreCowries, getLegalActions, validateRoll, applyRoll, validateAction, applyAction,
  rollLocalCowries, getPlayerSummary, describeTurn, resultTitle, resultDetail, actionSummary, assertStateInvariant
};
