...
const GAME_ID = FISHFLOW_RULESET.gameId;
const CURRENTS = ["blue", "coral"];

function createFishflowService(deps) {
  const {
    rooms,
    profiles,
    sockets,
  ...
  function oppositeCurrent(current) {
    return current === "blue" ? "coral" : "blue";
  }

  function isFishflowRoom(room) {
  ...
  function findRoom(ws, requestId, payload) {
    ...
    if (!room || !isFishflowRoom(room)) {
      fail(ws, requestId, "Fishflow room not found.");
      return null;
    }
    return room;
  }
  ...
  function broadcast(room) {
    ...
    saveRoomSafe(room);
    const packet = { type: "fish_room_state", payload: { room: roomView(room) } };
  ...
  function createRoom(ws, requestId, payload = {}) {
    ...
    if (!wallet) return;
    const chosenCurrent = normalizeCurrent(payload.current || payload.role);
    const roomCode = code();
    const now = Date.now();
    const room = {
      id: randomUUID(),
      ...
      updatedAt: now,
      players: {
        [wallet]: { wallet, seat: chosenCurrent, joinedAt: now, entryLocked: true }
      },
      gameState: createFishflowState({ mode: "online", starter: "blue" }),
      auditLog: []
    };
    audit(room, { type: "room_created", wallet, current: chosenCurrent, visibility: room.visibility });
    rooms.set(roomCode, room);
    saveRoomSafe(room);
    ok(ws, requestId, "fish_room_create_result", { room: roomView(room) });
    broadcast(room);
  }

  function listRooms(ws, requestId) {
    const result = [...rooms.values()]
      .filter(isFishflowRoom)
      .filter((room) => room.visibility === "public" && room.status === "waiting" && players(room).length < 2)
      .sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0))
      .map(roomView);
    ok(ws, requestId, "fish_room_list_result", { rooms: result });
  }
  ...
  function joinRoom(ws, requestId, payload = {}) {
    ...
    if (room.players[wallet]) {
      ok(ws, requestId, "fish_room_join_result", { room: roomView(room) });
      return;
    }
    ...
    const occupiedCurrent = players(room)[0]?.seat || "blue";
    const current = oppositeCurrent(occupiedCurrent);
    room.players[wallet] = { wallet, seat: current, joinedAt: Date.now(), entryLocked: true };
    room.status = "playing";
    room.startedAt = Date.now();
    audit(room, { type: "player_joined", wallet, current });
    audit(room, {
      type: "game_started",
      players: players(room).map((player) => ({ wallet: player.wallet, current: player.seat }))
    });
    ok(ws, requestId, "fish_room_join_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  function getState(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "fish_game_state_result", { room: roomView(room) });
  }
  ...
  function legalActions(ws, requestId, payload = {}) {
    ...
    const actions = room.status === "playing" && player.seat === room.gameState.currentPlayer
      ...
      : [];
    ok(ws, requestId, "fish_legal_actions_result", { actions, room: roomView(room) });
  }
  ...
  function action(ws, requestId, payload = {}) {
    ...
    if (room.status !== "playing") {
      return fail(ws, requestId, room.status === "finished" ? "The Fishflow match has ended." : "Waiting for a second player.");
    }
    ...
    audit(room, {
      ...
      summary: room.gameState.lastTurn
    });
    if (room.gameState.winner) finalize(room);
    ok(ws, requestId, "fish_game_action_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  async function history(ws, requestId, payload = {}) {
    ...
    if (!wallet) return;
    try {
      ok(ws, requestId, "fish_history_result", { history: await getHistoryForWallet(wallet, GAME_ID) });
    } catch (err) {
      fail(ws, requestId, err.message || "Could not load Fishflow history.");
    }
  }
  ...
  function myRooms(ws, requestId, payload = {}) {
    ...
    const result = [...rooms.values()]
      ...
      .map(roomView);
    ok(ws, requestId, "fish_my_rooms_result", { rooms: result });
  }
  ...
  function finalize(room) {
    ...
    const winnerCurrent = room.gameState.winner;
    const winnerPlayer = players(room).find((player) => player.seat === winnerCurrent) || null;
    room.result = {
  ...
  function recordHistory(room) {
    ...
    ps.forEach((player) => {
      ...
      saveHistoryEntry(entry).catch((err) => {
        console.error(`[fish-history] save failed room=${room.roomCode}: ${err.message}`);
      });
    });
    saveRoomSafe(room);
  }
  ...
  function restoreRoom(room) {
    if (!isFishflowRoom(room)) return false;
    room.rulesetVersion = room.rulesetVersion || FISHFLOW_RULESET.rulesetVersion;
...
module.exports = { createFishflowService, GAME_ID, CURRENTS };
