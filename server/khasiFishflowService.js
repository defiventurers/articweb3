...
const { KHASI_FISHFLOW_RULESET, applyAction, createKhasiFishflowState, getLegalActions, getPlayerSummary, assertStateInvariant } = require("./khasiFishflowRules.js");
...
function createKhasiFishflowService(deps) {
  ...
  function createRoom(ws, requestId, payload = {}) {
    ...
    const now = Date.now();
    const room = {
      id: randomUUID(), gameId: GAME_ID, rulesetVersion: KHASI_FISHFLOW_RULESET.rulesetVersion, roomCode,
      roomMode: "open_ice", visibility: payload.visibility === "private" ? "private" : "public", status: "waiting",
      matchId: `kf-${randomUUID()}`, contractMatchId: null, currency: null, entryWei: "0", createdAt: now, updatedAt: now,
  ...
  function joinRoom(ws, requestId, payload = {}) {
    ...
    const side = opposite(players(room)[0]?.seat || "aurora");
    room.players[wallet] = { wallet, seat: side, joinedAt: Date.now(), entryLocked: true };
    room.status = "playing"; room.startedAt = Date.now();
    audit(room, { type: "player_joined", wallet, side }); audit(room, { type: "game_started" });
  ...
  function action(ws, requestId, payload = {}) {
    ...
    const room = findRoom(ws, requestId, payload); if (!room) return;
    const player = room.players[wallet]; if (!player) return fail(ws, requestId, "Join the room first.");
    if (room.status !== "playing") return fail(ws, requestId, room.status === "finished" ? "This Mawkar Katiya match has ended." : "Waiting for a second player.");
    ...
    audit(room, { type: "game_action", wallet, side: player.seat, action: payload.action, resolved: room.gameState.lastMove, turn: room.gameState.turn, round: room.gameState.round });
  ...
  function finalize(room) {
    ...
    room.result = { winner: winnerSide, winnerWallet: winnerPlayer?.wallet || null, reason: room.gameState.winReason, rounds: room.gameState.round, captures: room.gameState.stores, reserves: room.gameState.reserves };
  ...
  function restoreRoom(room) {
    ...
    room.rulesetVersion = room.rulesetVersion || room.gameState.rulesetVersion;
    room.auditLog = room.auditLog || [];
