...
const GAME_ID = FOUR_WING_ICE_HUNT_RULESET.gameId;
const ROLES = ["leopards", "cattle"];

function createFourWingIceHuntService(deps) {
  ...
  function normalizeRole(value) { return value === "cattle" ? "cattle" : "leopards"; }
  function oppositeRole(role) { return role === "leopards" ? "cattle" : "leopards"; }
  function isFourWingRoom(room) { return room?.gameId === GAME_ID; }
  function code() {
  ...
  function findRoom(ws, requestId, payload) {
    ...
    if (!room || !isFourWingRoom(room)) {
      fail(ws, requestId, "Four-Wing Ice Hunt room not found.");
      return null;
    }
    return room;
  }
  ...
  function broadcast(room) {
    ...
    saveRoomSafe(room);
    const packet = { type: "fwh_room_state", payload: { room: roomView(room) } };
  ...
  function joinRoom(ws, requestId, payload = {}) {
    ...
    if (room.players[wallet]) {
      ok(ws, requestId, "fwh_room_join_result", { room: roomView(room) });
      return;
    }
    ...
    const occupiedRole = players(room)[0]?.seat || "leopards";
  ...
  function getState(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "fwh_game_state_result", { room: roomView(room) });
  }
  ...
  function legalActions(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "fwh_legal_actions_result", { actions, room: roomView(room) });
  }
  ...
  function action(ws, requestId, payload = {}) {
    ...
    const player = room.players[wallet];
    if (!player) return fail(ws, requestId, "Join the room first.");
    if (room.status !== "playing") return fail(ws, requestId, room.status === "finished" ? "The hunt has ended." : "Waiting for a second player.");
    ...
    ok(ws, requestId, "fwh_game_action_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  async function history(ws, requestId, payload = {}) {
    ...
    if (!wallet) return;
    try {
      ok(ws, requestId, "fwh_history_result", { history: await getHistoryForWallet(wallet, GAME_ID) });
    } catch (err) {
      fail(ws, requestId, err.message || "Could not load Four-Wing Ice Hunt history.");
    }
  }
  ...
  function myRooms(ws, requestId, payload = {}) {
    ...
    const result = [...rooms.values()]
      ...
      .map(roomView);
    ok(ws, requestId, "fwh_my_rooms_result", { rooms: result });
  }
  ...
  function finalize(room) {
    ...
    room.result = { winner: winnerRole, winnerWallet: winnerPlayer?.wallet || null, reason: room.gameState.winReason };
  ...
  function recordHistory(room) {
    ...
    ps.forEach((player) => {
      ...
      saveHistoryEntry(entry).catch((err) => console.error(`[fwh-history] save failed room=${room.roomCode}: ${err.message}`));
    });
    saveRoomSafe(room);
  }

  function restoreRoom(room) {
    if (!isFourWingRoom(room)) return false;
    room.rulesetVersion = room.rulesetVersion || FOUR_WING_ICE_HUNT_RULESET.rulesetVersion;
  ...
  return { createRoom, listRooms, joinRoom, getState, legalActions, action, history, myRooms, restoreRoom, roomView, isFourWingRoom };
}

module.exports = { createFourWingIceHuntService, GAME_ID, ROLES };
