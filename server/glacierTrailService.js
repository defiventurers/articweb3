...
const GAME_ID = GLACIER_TRAIL_RULESET.gameId;
const SIDES = ["aurora", "ember"];

function createGlacierTrailService(deps) {
  const {
    rooms,
    profiles,
    sockets,
    ...
    saveHistoryEntry,
    getHistoryForWallet,
    rollSequence = secureCowrieSequence
  } = deps;
  ...
  function isGlacierTrailRoom(room) { return room?.gameId === GAME_ID; }
  function code() {
  ...
  function findRoom(ws, requestId, payload) {
    ...
    if (!room || !isGlacierTrailRoom(room)) {
      fail(ws, requestId, "Glacier Trail room not found.");
      return null;
    }
    return room;
  }
  ...
  function broadcast(room) {
    ...
    saveRoomSafe(room);
    const packet = { type: "gt_room_state", payload: { room: roomView(room) } };
  ...
  function joinRoom(ws, requestId, payload = {}) {
    ...
    if (!room) return;
    if (room.players[wallet]) return ok(ws, requestId, "gt_room_join_result", { room: roomView(room) });
    ...
    ok(ws, requestId, "gt_room_join_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  function getState(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "gt_game_state_result", { room: roomView(room) });
  }
  ...
  function legalActions(ws, requestId, payload = {}) {
    ...
    const actions = room.status === "playing" && player.seat === room.gameState.currentPlayer
      ...
      : [];
    ok(ws, requestId, "gt_legal_actions_result", { actions, room: roomView(room) });
  }
  ...
  function roll(ws, requestId, payload = {}) {
    ...
    const player = room.players[wallet];
    if (!player) return fail(ws, requestId, "Join the room first.");
    if (room.status !== "playing") return fail(ws, requestId, room.status === "finished" ? "The Glacier Trail race has ended." : "Waiting for a second player.");
    if (player.seat !== room.gameState.currentPlayer) return fail(ws, requestId, "It is not your turn.");
    if (room.gameState.awaiting !== "roll") return fail(ws, requestId, "Allocate the stored cowrie throws first.");

    const sequence = rollSequence();
    const nonce = randomUUID();
    const proofHash = sha256(JSON.stringify({ matchId: room.matchId, turn: room.gameState.turn, castCount: room.gameState.castCount, sequence, nonce }));
    const result = applyRollSequence(room.gameState, sequence, player.seat, { nonce, proofHash });
    if (result.error) return fail(ws, requestId, result.error);
    room.gameState = result.state;
    audit(room, { type: "cowrie_sequence", wallet, side: player.seat, sequence, proofHash, nonce });
    ok(ws, requestId, "gt_game_roll_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  function action(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "gt_game_action_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  async function history(ws, requestId, payload = {}) {
    ...
    if (!wallet) return;
    try {
      ok(ws, requestId, "gt_history_result", { history: await getHistoryForWallet(wallet, GAME_ID) });
    } catch (err) {
      fail(ws, requestId, err.message || "Could not load Glacier Trail history.");
    }
  }
  ...
  function myRooms(ws, requestId, payload = {}) {
    ...
    const result = [...rooms.values()]
      ...
      .map(roomView);
    ok(ws, requestId, "gt_my_rooms_result", { rooms: result });
  }
  ...
  function recordHistory(room) {
    ...
    ps.forEach((player) => {
      ...
      saveHistoryEntry(entry).catch((err) => console.error(`[gt-history] save failed room=${room.roomCode}: ${err.message}`));
    });
    saveRoomSafe(room);
  }

  function restoreRoom(room) {
    if (!isGlacierTrailRoom(room)) return false;
    room.rulesetVersion = room.rulesetVersion || GLACIER_TRAIL_RULESET.rulesetVersion;
  ...
  return { action, createRoom, getState, history, isGlacierTrailRoom, joinRoom, legalActions, listRooms, myRooms, restoreRoom, roll, roomView };
}

function secureCowrieSequence() {
  const sequence = [];
  do {
    const faces = Array.from({ length: GLACIER_TRAIL_RULESET.cowries }, () => randomInt(0, 2));
...
module.exports = { createGlacierTrailService, secureCowrieSequence, GAME_ID, SIDES };
