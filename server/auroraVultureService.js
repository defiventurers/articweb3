...
const GAME_ID = AURORA_VULTURE_RULESET.gameId;

function createAuroraVultureService(deps) {
  const {
    rooms,
    profiles,
    sockets,
  ...
  const normalizeSide = (value) => value === "vulture" ? "vulture" : "crows";
  const oppositeSide = (side) => side === "crows" ? "vulture" : "crows";
  ...
  function findRoom(ws, requestId, payload) {
    ...
    if (!room || !isRoom(room)) {
      fail(ws, requestId, "Aurora Vulture room not found.");
      return null;
    }
    return room;
  }
  ...
  function broadcast(room) {
    ...
    saveRoomSafe(room);
    const packet = { type: "av_room_state", payload: { room: roomView(room) } };
  ...
  function createRoom(ws, requestId, payload = {}) {
    ...
    const room = {
      ...
      gameState: createAuroraVultureState({ mode: "online" }),
      auditLog: []
    };
    audit(room, { type: "room_created", wallet, side: chosenSide, visibility: room.visibility });
    rooms.set(roomCode, room);
    saveRoomSafe(room);
    ok(ws, requestId, "av_room_create_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  function listRooms(ws, requestId) {
    const result = [...rooms.values()]
      ...
      .map(roomView);
    ok(ws, requestId, "av_room_list_result", { rooms: result });
  }
  ...
  function joinRoom(ws, requestId, payload = {}) {
    ...
    if (!room) return;
    if (room.players[wallet]) return ok(ws, requestId, "av_room_join_result", { room: roomView(room) });
    ...
    const occupied = players(room)[0]?.seat || "crows";
    ...
    ok(ws, requestId, "av_room_join_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  function getState(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "av_game_state_result", { room: roomView(room) });
  }
  ...
  function legalActions(ws, requestId, payload = {}) {
    ...
    const actions = room.status === "playing" && player.seat === room.gameState.currentPlayer
      ...
      : [];
    ok(ws, requestId, "av_legal_actions_result", { actions, room: roomView(room) });
  }
  ...
  function action(ws, requestId, payload = {}) {
    ...
    const player = room.players[wallet];
    if (!player) return fail(ws, requestId, "Join the room first.");
    if (room.status !== "playing") return fail(ws, requestId, room.status === "finished" ? "This Aurora Vulture match has ended." : "Waiting for a second player.");
    ...
    audit(room, {
      ...
      resolved: room.gameState.lastAction,
      turn: room.gameState.turn
    });
    if (room.gameState.winner || room.gameState.isDraw) finalize(room);
    ok(ws, requestId, "av_game_action_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  async function history(ws, requestId, payload = {}) {
    ...
    if (!wallet) return;
    try {
      ok(ws, requestId, "av_history_result", { history: await getHistoryForWallet(wallet, GAME_ID) });
    } catch (err) {
      fail(ws, requestId, err.message || "Could not load Aurora Vulture history.");
    }
  }
  ...
  function myRooms(ws, requestId, payload = {}) {
    ...
    const result = [...rooms.values()]
      ...
      .map(roomView);
    ok(ws, requestId, "av_my_rooms_result", { rooms: result });
  }
  ...
  function finalize(room) {
    ...
    const winnerPlayer = winnerSide ? players(room).find((player) => player.seat === winnerSide) || null : null;
    room.result = {
      winner: winnerSide,
      winnerWallet: winnerPlayer?.wallet || null,
      draw: Boolean(room.gameState.isDraw),
      reason: room.gameState.winReason,
      plies: room.gameState.ply,
      movementPlies: room.gameState.movementPly,
      capturedCrows: room.gameState.capturedCrows
    };
  ...
  function recordHistory(room) {
    ...
    const ps = players(room);
    const summaries = ps.map((player) => ({
      wallet: player.wallet,
      name: profileFor(player.wallet)?.name || "Player",
      role: player.seat,
      won: room.result?.winnerWallet === player.wallet,
      draw: Boolean(room.result?.draw),
    ...
    ps.forEach((player) => {
      ...
      const entry = {
        ...
        team: player.seat,
        position: room.result?.draw ? 1 : won ? 1 : 2,
        won,
        draw: Boolean(room.result?.draw),
        payoutWei: "0",
        points: won ? 14 : room.result?.draw ? 4 : 2,
  ...
  function restoreRoom(room) {
    ...
    room.rulesetVersion = room.rulesetVersion || AURORA_VULTURE_RULESET.rulesetVersion;
    room.gameState = room.gameState || createAuroraVultureState({ mode: "online" });
    room.auditLog = room.auditLog || [];
...
module.exports = { createAuroraVultureService };
