const BREAK_THE_ICE_RULESET = Object.freeze({
...
const PLAYERS = Object.freeze(["blue", "coral"]);
...
function createBreakTheIceState({ mode = "hotseat", starter = "blue", seed = Date.now() } = {}) {
  return {
...
function routeFor(player) {
  return ROUTES[player] || ROUTES.blue;
}

function getPiece(state, player, pieceId) {
...
function getLegalActions(state, player = state.currentPlayer) {
  ...
  if (BREAK_THE_ICE_RULESET.entryValues.includes(value)) {
    const startSpace = route[0];
    if (canLand(state, player, startSpace)) {
      for (const piece of state.pieces[player]) {
        if (piece.status === "home") actions.push({ type: "enter", pieceId: piece.id, targetProgress: 0, targetSpace: startSpace });
      }
    }
  }
...
function validateRoll(state, faces, player = state.currentPlayer) {
  ...
  if (!Array.isArray(faces) || faces.length !== BREAK_THE_ICE_RULESET.cowries || faces.some((face) => ![0, 1, false, true].includes(face))) {
    return { valid: false, reason: "A cowrie throw must contain seven open-or-closed shells." };
  }
  return { valid: true };
}

function applyRoll(state, faces, player = state.currentPlayer, meta = {}) {
...
function validateAction(state, action, player = state.currentPlayer) {
  ...
  if (!action || !["enter", "move"].includes(action.type) || typeof action.pieceId !== "string") {
    return { valid: false, reason: "Choose one legal penguin runner." };
  }
...
function applyAction(state, action, player = state.currentPlayer) {
  ...
  next.lastMove = {
    ...
    player,
    pieceId: piece.id,
    value: next.roll.value,
    fromSpace,
    targetSpace: legalAction.targetSpace,
    targetProgress: legalAction.targetProgress,
...
function rollLocalCowries(state, player = state.currentPlayer) {
...
function getPlayerSummary(state, player) {
...
function describeTurn(state) {
  ...
  if (state.awaiting === "roll") return `${label}: cast seven cowries.`;
  return `${label}: move one legal runner by ${state.roll?.value || 0}.`;
}
...
function resultDetail(state) {
  return state.winReason === "all-five-finished"
    ? "All five penguin runners crossed beyond the final marked space with exact throws."
    : "The Break the Ice race is complete.";
}
...
function explainIllegalAction(state, action, player) {
  ...
  if (piece.status === "home" && !BREAK_THE_ICE_RULESET.entryValues.includes(Number(state.roll?.value || 0))) {
    return "A waiting runner may enter only on a throw of 1, 5, or 7.";
  }
  if (piece.status === "track" && piece.progress + Number(state.roll?.value || 0) > FINISH_PROGRESS) {
    return "The final exit requires an exact throw.";
  }
...
function assertStateInvariant(state) {
  ...
  if (allPieces.length !== BREAK_THE_ICE_RULESET.piecesPerPlayer * PLAYERS.length) {
    throw new Error("Break the Ice piece-count invariant failed.");
  }
