...
const GAME_ID = SIXTEEN_ICE_WARRIORS_RULESET.gameId;
const ROLES = ["aurora", "ember"];

function createSixteenIceWarriorsService(deps) {
  ...
  function isSixteenIceWarriorsRoom(room) { return room?.gameId === GAME_ID; }
  function code() {
  ...
  function findRoom(ws, requestId, payload) {
    ...
    if (!room || !isSixteenIceWarriorsRoom(room)) {
      fail(ws, requestId, "Sixteen Ice Warriors room not found.");
      return null;
    }
    return room;
  }
  ...
  function broadcast(room) {
    ...
    saveRoomSafe(room);
    const packet = { type: "siw_room_state", payload: { room: roomView(room) } };
  ...
  function joinRoom(ws, requestId, payload = {}) {
    ...
    if (room.players[wallet]) {
      ok(ws, requestId, "siw_room_join_result", { room: roomView(room) });
      return;
    }
  ...
  function getState(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "siw_game_state_result", { room: roomView(room) });
  }
  ...
  function legalActions(ws, requestId, payload = {}) {
    ...
    const actions = room.status === "playing" && player.seat === room.gameState.currentPlayer
      ...
      : [];
    ok(ws, requestId, "siw_legal_actions_result", { actions, room: roomView(room) });
  }
  ...
  function action(ws, requestId, payload = {}) {
    ...
    const player = room.players[wallet];
    if (!player) return fail(ws, requestId, "Join the room first.");
    if (room.status !== "playing") return fail(ws, requestId, room.status === "finished" ? "The battle has ended." : "Waiting for a second player.");
    ...
    ok(ws, requestId, "siw_game_action_result", { room: roomView(room) });
    broadcast(room);
  }
  ...
  async function history(ws, requestId, payload = {}) {
    ...
    if (!wallet) return;
    try {
      ok(ws, requestId, "siw_history_result", { history: await getHistoryForWallet(wallet, GAME_ID) });
    } catch (err) {
      fail(ws, requestId, err.message || "Could not load Sixteen Ice Warriors history.");
    }
  }
  ...
  function myRooms(ws, requestId, payload = {}) {
    ...
    const result = [...rooms.values()]
      ...
      .map(roomView);
    ok(ws, requestId, "siw_my_rooms_result", { rooms: result });
  }
  ...
  function recordHistory(room) {
    ...
    ps.forEach((player) => {
      ...
      saveHistoryEntry(entry).catch((err) => console.error(`[siw-history] save failed room=${room.roomCode}: ${err.message}`));
    });
    saveRoomSafe(room);
  }

  function restoreRoom(room) {
    if (!isSixteenIceWarriorsRoom(room)) return false;
    room.rulesetVersion = room.rulesetVersion || SIXTEEN_ICE_WARRIORS_RULESET.rulesetVersion;
  ...
  return {
    ...
    restoreRoom,
    roomView,
    isSixteenIceWarriorsRoom
  };
}

module.exports = { createSixteenIceWarriorsService, GAME_ID, ROLES };
