const SIGE_RULESET = Object.freeze({
  gameId: "sige",
...
const CELLS = Object.freeze(Array.from({ length: 25 }, (_, index) => ({ id: cellId(Math.floor(index / 5), index % 5), row: Math.floor(index / 5), col: index % 5 })));
const SAFE_SPACES = Object.freeze(new Set(SIGE_RULESET.safeSpaces));
...
const ROUTES = Object.freeze({ aurora: AURORA_ROUTE, ember: Object.freeze(AURORA_ROUTE.map(rotate180)) });
const FINISH_PROGRESS = 24;

function createPieces(side) {
  return Array.from({ length: 2 }, (_, index) => ({ id: `${side}-${index + 1}`, side, status: "home", progress: -1 }));
}

function createSigeState({ mode = "online", starter = "aurora", seed = Date.now() } = {}) {
  const state = {
...
function createSplitFinishDrill() {
...
function getOccupants(state, spaceId, ignoreIds = []) {
...
function resultTitle(state) { return state.winner === "ember" ? "Ember Route reaches the centre" : state.winner === "aurora" ? "Aurora Route reaches the centre" : "Sige race in progress"; }
function resultDetail(state) { return state.winReason === "split-centre-finish" ? "One exact cowrie throw was divided only at the centre to finish both counters." : "Both counters reached the protected centre by exact movement."; }
...
function sideName(side) { return side === "ember" ? "Ember Route" : "Aurora Route"; }
...
module.exports = {
  SIGE_RULESET, SIDES, BOARD_SIZE, CELLS, SAFE_SPACES, ROUTES, FINISH_PROGRESS, cellId,
  createSigeState, createSplitFinishDrill, otherSide, routeFor, getPiece, getPieceSpaceId, getOccupants,
  scoreCowries, getLegalActions, validateRoll, applyRoll, validateAction, applyAction, rollLocalCowries,
  getPlayerSummary, describeTurn, resultTitle, resultDetail, actionSummary, assertStateInvariant
};
