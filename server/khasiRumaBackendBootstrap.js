...
function injectKhasiFishflow(source) {
  ...
  const requireNeedle = 'const { createSevenIceRingsService } = require("./sevenIceRingsService.js");';
  if (!transformed.includes('require("./khasiFishflowService.js")')) {
    if (!transformed.includes(requireNeedle)) throw new Error("Unable to locate Seven Ice Rings service require for Khasi Fishflow.");
    transformed = transformed.replace(requireNeedle, `${requireNeedle}\nconst { createKhasiFishflowService } = require("./khasiFishflowService.js");`);
  }

  const initNeedle = `const sevenIceRings = createSevenIceRingsService({
  rooms,
  profiles,
  sockets,
  send,
...
const khasiFishflow = createKhasiFishflowService({
  ...
  if (!transformed.includes("const khasiFishflow = createKhasiFishflowService")) {
    if (!transformed.includes(initNeedle)) throw new Error("Unable to locate Seven Ice Rings initializer for Khasi Fishflow.");
    transformed = transformed.replace(initNeedle, initReplacement);
  }
...
function installKhasiRumaBackend() { multiGameBackend.loadMultiGameBackend = loadKhasiRumaBackend; return multiGameBackend; }
installKhasiRumaBackend();
module.exports = { injectKhasiFishflow, loadKhasiRumaBackend, installKhasiRumaBackend };
