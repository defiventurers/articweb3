...
function injectKhasiFishflowAfterAllGames(source) {
  ...
  const sevenIceRingsInit = `const sevenIceRings = createSevenIceRingsService({
  rooms,
  profiles,
  sockets,
  send,
  ...
  const khasiFishflowInit = `${sevenIceRingsInit}
const khasiFishflow = createKhasiFishflowService({
  ...
  if (!transformed.includes("const khasiFishflow = createKhasiFishflowService")) {
    if (!transformed.includes(sevenIceRingsInit)) throw new Error("Unable to locate Seven Ice Rings initializer for Khasi Fishflow.");
    transformed = transformed.replace(sevenIceRingsInit, khasiFishflowInit);
  }
  ...
  if (!transformed.includes('"khasi-fishflow"].includes(room?.gameId)')) {
    if (!transformed.includes(scheduleNeedle)) throw new Error("Unable to locate heritage-game scheduler guard for Khasi Fishflow.");
  ...
  if (!transformed.includes("khasiFishflow.restoreRoom(room)")) {
    if (!transformed.includes(restoreNeedle)) throw new Error("Unable to locate full restore loop for Khasi Fishflow.");
  ...
  if (!transformed.includes("supportsKhasiFishflow: true")) {
    if (!transformed.includes(healthNeedle)) throw new Error("Unable to locate Seven Ice Rings health capability for Khasi Fishflow.");
...
function loadAllGamesBackend() {
  const indexPath = require.resolve("./index.js");
  if (require.cache[indexPath]) return require.cache[indexPath].exports;
  const source = fs.readFileSync(indexPath, "utf8");
  const transformed = injectKhasiFishflowAfterAllGames(
    sevenRingsBackend.injectSevenIceRings(
...
function installAllGamesBackend() {
  multiGameBackend.loadMultiGameBackend = loadAllGamesBackend;
  return multiGameBackend;
}

installAllGamesBackend();
module.exports = { injectKhasiFishflowAfterAllGames, loadAllGamesBackend, installAllGamesBackend };
