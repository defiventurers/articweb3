const AURORA_VULTURE_RULESET = Object.freeze({
...
const SIDES = Object.freeze(["crows", "vulture"]);
...
const EDGES = Object.freeze(uniquePairs(LINES.flatMap((line) => line.slice(0, -1).map((point, index) => [point, line[index + 1]]))));
const ADJACENCY = Object.freeze(Object.fromEntries(POINTS.map((point) => [point.id, Object.freeze(EDGES.flatMap(([a, b]) => a === point.id ? [b] : b === point.id ? [a] : []))])));
const JUMPS = Object.freeze(Object.fromEntries(POINTS.map((point) => [point.id, Object.freeze(jumpsFrom(point.id))])));

function createAuroraVultureState({ mode = "online" } = {}) {
  const state = {
    ...
    crows: Array.from({ length: 7 }, (_, index) => ({ id: `crow-${index + 1}`, side: "crows", point: null, status: "waiting" })),
    deployedCrows: 0,
    capturedCrows: 0,
    turn: 1,
    ply: 0,
...
function createFourthCrowDrill() {
  ...
  const active = [["crow-1", "i0"], ["crow-2", "o1"], ["crow-3", "o3"], ["crow-4", "o4"]];
  state.crows.forEach((crow, index) => {
    const setup = active.find(([id]) => id === crow.id);
    if (setup) { crow.point = setup[1]; crow.status = "board"; }
    else if (index >= 4) { crow.point = null; crow.status = "captured"; }
  });
...
function otherSide(side) { return side === "crows" ? "vulture" : "crows"; }
function occupantAt(state, pointId) {
...
function onBoardPieces(state) { return [state.vulture, ...state.crows].filter((piece) => piece.status === "board" && piece.point); }
...
function getLegalActions(state, side = state.currentPlayer) {
  ...
  if (side === "crows") {
    if (state.deployedCrows < 7) {
      const nextCrow = state.crows.find((crow) => crow.status === "waiting");
      if (!nextCrow) return [];
      return POINTS.filter((point) => !occupied.has(point.id)).map((point) => ({ type: "place-crow", pieceId: nextCrow.id, to: point.id }));
    }
    const actions = [];
    for (const crow of state.crows.filter((piece) => piece.status === "board")) {
      for (const to of ADJACENCY[crow.point] || []) if (!occupied.has(to)) actions.push({ type: "move-crow", pieceId: crow.id, from: crow.point, to });
    }
    return actions;
  }
  if (!state.vulture.point) return POINTS.filter((point) => !occupied.has(point.id)).map((point) => ({ type: "place-vulture", pieceId: state.vulture.id, to: point.id }));
  const actions = [];
  for (const to of ADJACENCY[state.vulture.point] || []) if (!occupied.has(to)) actions.push({ type: "move-vulture", pieceId: state.vulture.id, from: state.vulture.point, to });
  for (const jump of JUMPS[state.vulture.point] || []) {
    const jumped = occupantAt(state, jump.over);
    if (jumped?.side === "crows" && !occupied.has(jump.to)) actions.push({ type: "capture-crow", pieceId: state.vulture.id, from: state.vulture.point, over: jump.over, to: jump.to, capturedPieceId: jumped.id });
  }
  return actions;
}
...
function applyAction(state, action, side = state.currentPlayer) {
  ...
  if (legal.type === "place-crow") {
    ...
    crow.point = legal.to; crow.status = "board"; next.deployedCrows += 1;
  } else if (legal.type === "place-vulture") {
    next.vulture.point = legal.to; next.vulture.status = "board";
  } else if (legal.type === "move-crow") {
    const crow = next.crows.find((piece) => piece.id === legal.pieceId);
    from = crow.point; crow.point = legal.to;
  } else if (legal.type === "move-vulture") {
    from = next.vulture.point; next.vulture.point = legal.to;
  } else if (legal.type === "capture-crow") {
    from = next.vulture.point; next.vulture.point = legal.to;
    const crow = next.crows.find((piece) => piece.id === legal.capturedPieceId);
    crow.point = null; crow.status = "captured"; next.capturedCrows += 1;
  }
  next.ply += 1;
  ...
  if (next.capturedCrows >= 4) {
    next.winner = "vulture"; next.winReason = "four-crows-captured";
  } else {
    next.currentPlayer = otherSide(side); next.turn += 1;
    if (next.deployedCrows === 7 && next.vulture.point) next.phase = "movement";
    if (next.currentPlayer === "vulture" && !getLegalActions(next, "vulture").length) {
      next.winner = "crows"; next.winReason = "vulture-immobilized";
    }
  }
  if (!next.winner && next.phase === "movement") {
    const key = positionKey(next);
    next.repetitions[key] = Number(next.repetitions[key] || 0) + 1;
    if (next.repetitions[key] >= 3) { next.isDraw = true; next.winReason = "threefold-repetition"; }
    else if (next.movementPly >= 80) { next.isDraw = true; next.winReason = "movement-ply-limit"; }
  }
...
function getPlayerSummary(state, side) {
  if (side === "vulture") return { captures: state.capturedCrows, captureTarget: 4, point: state.vulture.point };
  return { deployed: state.deployedCrows, onBoard: state.crows.filter((crow) => crow.status === "board").length, waiting: state.crows.filter((crow) => crow.status === "waiting").length, captured: state.capturedCrows };
}
function describeTurn(state) {
  if (state.winner || state.isDraw) return resultTitle(state);
  if (state.currentPlayer === "crows") return state.deployedCrows < 7 ? `Aurora Crows: place defender ${state.deployedCrows + 1} of 7.` : "Aurora Crows: move one defender to tighten the ring.";
  return state.vulture.point ? "Glacier Vulture: move or make one straight jump capture." : "Glacier Vulture: choose an empty star point.";
}
...
function resultDetail(state) {
  ...
  if (state.winReason === "movement-ply-limit") return "No victory was completed within 80 movement plies.";
  return "The Kaooa match is complete.";
}
function assertStateInvariant(state) {
  if (!state || state.crows?.length !== 7) throw new Error("Aurora Vulture must contain seven crows.");
...
function jumpsFrom(pointId) {
  const jumps = [];
  for (const line of LINES) for (let index = 0; index < line.length - 2; index += 1) {
...
function uniquePairs(pairs) {
  ...
  for (const pair of pairs) { const key = [...pair].sort().join("|"); if (seen.has(key)) continue; seen.add(key); result.push(Object.freeze(pair)); }
  return result;
}
function actionKey(action) { return `${action?.type || ""}:${action?.pieceId || ""}:${action?.from || ""}:${action?.over || ""}:${action?.to || ""}:${action?.capturedPieceId || ""}`; }
function pointName(pointId) { return POINTS.find((point) => point.id === pointId)?.label || pointId; }
