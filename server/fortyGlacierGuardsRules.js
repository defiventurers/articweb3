const FORTY_GLACIER_GUARDS_RULESET = Object.freeze({
...
const BOARD_SIZE = FORTY_GLACIER_GUARDS_RULESET.boardSize;
const NODES = Object.freeze(Array.from({ length: BOARD_SIZE }, (_, row) =>
...
const CENTER_NODE = nodeId(4, 4);

function nodeId(row, col) {
  return `g${row}${col}`;
}

function createFortyGlacierGuardsState({ mode = "online", starter = "aurora" } = {}) {
...
function createBreakthroughDrillState() {
