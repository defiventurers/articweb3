const ICE_RINGS_RULESET = Object.freeze({
  gameId: "ice-rings",
...
function ringNodeId(ring, sector) { return `r${ring}s${sector}`; }
function pointFor(ring, sector) {
  const radii = [0, 17, 30, 43];
  const angle = -Math.PI / 2 + (sector * TAU) / ICE_RINGS_RULESET.sectors;
  return { x: 50 + Math.cos(angle) * radii[ring], y: 50 + Math.sin(angle) * radii[ring] };
}
...
const NODES = Object.freeze([
  ...
  ...Array.from({ length: ICE_RINGS_RULESET.rings }, (_, ringOffset) => {
    ...
    return Array.from({ length: ICE_RINGS_RULESET.sectors }, (_, sector) => ({ id: ringNodeId(ring, sector), ...pointFor(ring, sector), ring, sector, label: `ring ${ring}, spoke ${sector + 1}` }));
  }).flat()
]);
const NODE_BY_ID = Object.freeze(Object.fromEntries(NODES.map((node) => [node.id, node])));
const NODE_IDS = new Set(NODES.map((node) => node.id));
const EDGES = Object.freeze(buildEdges());
const JUMP_PATHS = Object.freeze(buildJumpPaths());
const ADJACENCY = Object.freeze(EDGES.reduce((map, [a, b]) => {
  map[a] = [...(map[a] || []), b];
  map[b] = [...(map[b] || []), a];
  return map;
}, {}));
const AURORA_START = Object.freeze([0, 1, 2].flatMap((sector) => [1, 2, 3].map((ring) => ringNodeId(ring, sector))));
const EMBER_START = Object.freeze([3, 4, 5].flatMap((sector) => [1, 2, 3].map((ring) => ringNodeId(ring, sector))));

function buildEdges() {
  const edges = [];
  for (let ring = 1; ring <= 3; ring += 1) {
    for (let sector = 0; sector < 6; sector += 1) edges.push([ringNodeId(ring, sector), ringNodeId(ring, (sector + 1) % 6)]);
  }
...
function buildJumpPaths() {
  ...
  for (let sector = 0; sector < 6; sector += 1) addLineTriples(paths, ["c", ringNodeId(1, sector), ringNodeId(2, sector), ringNodeId(3, sector)], `spoke-${sector}`);
...
function dedupePairs(pairs) {
  const seen = new Set();
  return pairs.filter(([a, b]) => { const key = [a, b].sort().join("|"); if (seen.has(key)) return false; seen.add(key); return true; });
}
function dedupePaths(paths) {
  const seen = new Set();
  return paths.filter((path) => { const key = `${path.from}|${path.over}|${path.to}`; if (seen.has(key)) return false; seen.add(key); return true; });
}

function createIceRingsState({ mode = "online", starter = "aurora" } = {}) {
...
function createRingBreakDrillState() {
...
function occupiedNodes(state, side) { return NODES.filter((node) => state.board[node.id] === side).map((node) => node.id); }
...
function getCaptureActions(state, side, from) {
  ...
  return JUMP_PATHS.filter((path) => path.from === from).filter((path) => state.board[path.over] === enemy && !state.board[path.to]).map((path) => ({ type: "capture", ...path }));
}
...
function applyAction(state, action, side = state.currentPlayer) {
  ...
  if (legal.type === "capture") {
    ...
    if (occupiedNodes(next, otherSide(side)).length === 0) {
      ...
      next.chainFrom = null;
    } else if (getCaptureActions(next, side, legal.to).length) next.chainFrom = legal.to;
...
function applyDrawPolicy(state) {
  ...
  if (state.positionCounts[signature] >= ICE_RINGS_RULESET.repetitionLimit) { state.winner = "draw"; state.winReason = "threefold-repetition"; }
  else if (state.noCapturePly >= ICE_RINGS_RULESET.noCapturePlyLimit) { state.winner = "draw"; state.winReason = "no-capture-limit"; }
}
...
function positionSignature(state) { return `${state.currentPlayer}|${NODES.map((node) => state.board[node.id] || "-").join("")}`; }
function getCounts(state) {
  return { auroraOnBoard: occupiedNodes(state, "aurora").length, emberOnBoard: occupiedNodes(state, "ember").length, auroraCaptured: Number(state.captured.aurora || 0), emberCaptured: Number(state.captured.ember || 0) };
}
...
function actionSummary(action) {
  ...
  return action.type === "capture" ? `${label} jumped ${action.over} and landed on ${action.to}.` : `${label} moved from ${action.from} to ${action.to}.`;
}
...
function resultDetail(state) {
  return { "all-rivals-captured": "Every opposing Pretwa guard has been captured.", "rival-immobilized": "The opposing formation has no legal step or capture.", "threefold-repetition": "The same completed-turn position occurred three times under the digital tournament policy.", "no-capture-limit": "One hundred captureless plies elapsed under the digital tournament policy." }[state.winReason] || "The circular battle is complete.";
}
function actionKey(action) { return action?.type === "capture" ? `capture:${action.from}:${action.over}:${action.to}` : `move:${action?.from}:${action?.to}`; }
...
function assertStateInvariant(state) {
  ...
  for (const node of NODES) if (![null, ...SIDES].includes(state.board[node.id])) throw new Error(`Invalid occupant at ${node.id}.`);
