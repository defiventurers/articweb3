...
const { SIGE_RULESET, applyAction, applyRoll, createSigeState, getLegalActions, getPlayerSummary, assertStateInvariant } = require("./sigeRules.js");

const GAME_ID = SIGE_RULESET.gameId;

function createSigeService(deps) {
  ...
  function findRoom(ws, requestId, payload) {
    ...
    if (!room || !isRoom(room)) { fail(ws, requestId, "Sige room not found."); return null; }
    return room;
  }
  ...
  function broadcast(room) {
    ...
    saveRoomSafe(room);
    const packet = { type: "sg_room_state", payload: { room: roomView(room) } };
  ...
  function createRoom(ws, requestId, payload = {}) {
    ...
    const room = {
      ...
      gameState: createSigeState({ mode: "online", starter: "aurora" }),
      auditLog: []
    };
    audit(room, { type: "room_created", wallet, side: chosenSide, visibility: room.visibility });
    rooms.set(roomCode, room);
    saveRoomSafe(room);
    ok(ws, requestId, "sg_room_create_result", { room: roomView(room) });
    broadcast(room);
  }
  function listRooms(ws, requestId) {
    ...
    ok(ws, requestId, "sg_room_list_result", { rooms: result });
  }
  function joinRoom(ws, requestId, payload = {}) {
    ...
    if (!room) return;
    if (room.players[wallet]) return ok(ws, requestId, "sg_room_join_result", { room: roomView(room) });
    ...
    const occupied = players(room)[0]?.seat || "aurora";
    const side = oppositeSide(occupied);
    ...
    ok(ws, requestId, "sg_room_join_result", { room: roomView(room) });
    broadcast(room);
  }
  function getState(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "sg_game_state_result", { room: roomView(room) });
  }
  function legalActions(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "sg_legal_actions_result", { actions, room: roomView(room) });
  }
  function roll(ws, requestId, payload = {}) {
    ...
    const player = room.players[wallet];
    if (!player) return fail(ws, requestId, "Join the room first.");
    if (room.status !== "playing") return fail(ws, requestId, room.status === "finished" ? "This Sige race has ended." : "Waiting for a second player.");
    if (player.seat !== room.gameState.currentPlayer) return fail(ws, requestId, "It is not your turn.");
    const nonce = randomBytes(16).toString("hex");
    const seed = randomBytes(1)[0];
    const faces = [0, 1, 2, 3].map((bit) => (seed >> bit) & 1);
    const proofHash = sha256(JSON.stringify({ gameId: GAME_ID, roomCode: room.roomCode, matchId: room.matchId, cast: room.gameState.castCount + 1, nonce, faces }));
    ...
    audit(room, { type: "cowrie_cast", wallet, side: player.seat, faces, value: room.gameState.lastRoll.value, nonce, proofHash });
    ok(ws, requestId, "sg_game_roll_result", { room: roomView(room) });
    broadcast(room);
  }
  function action(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "sg_game_action_result", { room: roomView(room) });
    broadcast(room);
  }
  async function history(ws, requestId, payload = {}) {
    const wallet = requireProfile(ws, requestId, payload);
    if (!wallet) return;
    try { ok(ws, requestId, "sg_history_result", { history: await getHistoryForWallet(wallet, GAME_ID) }); }
    catch (err) { fail(ws, requestId, err.message || "Could not load Sige history."); }
  }
  function myRooms(ws, requestId, payload = {}) {
    ...
    ok(ws, requestId, "sg_my_rooms_result", { rooms: result });
  }
  function finalize(room) {
    ...
    room.result = { winner: winnerSide, winnerWallet: winnerPlayer?.wallet || null, draw: false, reason: room.gameState.winReason, captures: room.gameState.captures, casts: room.gameState.castCount };
  ...
  function recordHistory(room) {
    ...
    const ps = players(room);
    const summaries = ps.map((player) => ({ wallet: player.wallet, name: profileFor(player.wallet)?.name || "Player", role: player.seat, won: room.result?.winnerWallet === player.wallet, summary: getPlayerSummary(room.gameState, player.seat) }));
    ps.forEach((player) => {
  ...
  function restoreRoom(room) {
    ...
    room.rulesetVersion = room.rulesetVersion || SIGE_RULESET.rulesetVersion;
    room.gameState = room.gameState || createSigeState({ mode: "online" });
    room.auditLog = room.auditLog || [];
    assertStateInvariant(room.gameState);
    return true;
  }

  return { createRoom, listRooms, joinRoom, getState, legalActions, roll, action, history, myRooms, restoreRoom, roomView };
}

module.exports = { createSigeService };
