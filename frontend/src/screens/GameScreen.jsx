...
function PieceImage({ piece, className }) { const color = TEAM_ASSET_COLOR[piece.team]; const type = PIECE_ASSET_TYPE[piece.type]; if (!color || !type) return <span>{PIECE_LETTER[piece.type] || "?"}</span>; const filename = `${color}-${type}.png`; const localSrc = `${LOCAL_PIECE_ASSET_BASE}/${filename}`; const remoteSrc = `${REMOTE_PIECE_ASSET_BASE}/${filename}`; return <img src={localSrc} alt={`${TEAM_LABEL[piece.team]} ${piece.type}`} className={className} draggable="false" onError={(event) => { if (event.currentTarget.src !== remoteSrc) event.currentTarget.src = remoteSrc; }} />; }
