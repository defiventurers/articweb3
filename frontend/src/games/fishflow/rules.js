export const FISHFLOW_RULESET = Object.freeze({
  gameId: "fishflow",
...
export function createFishflowState({ mode = "hotseat", starter = "blue" } = {}) {
...
export function scoreTurn(state, action, player = state.currentPlayer) {
  ...
  return (
    summary.captured * 100 +
    summary.exactFourPickups * 15 +
    summary.relays * 4 +
    (summary.roundEnded ? 40 : 0) +
    counts[player].total - counts[otherPlayer(player)].total
  );
}

export function createDailyFishflowPuzzle(dateKey = new Date().toISOString().slice(0, 10)) {
  let seed = hashString(dateKey);
  let state = createFishflowState({ mode: "daily", starter: seed % 2 ? "coral" : "blue" });
  const steps = 5 + (seed % 7);
  for (let step = 0; step < steps; step += 1) {
    const actions = getLegalActions(state);
    if (actions.length < 2 || state.winner) break;
    seed = nextSeed(seed);
    const action = actions[seed % actions.length];
    const result = applyAction(state, action, state.currentPlayer);
    if (result.error || result.state.round !== 1 || result.state.winner) break;
    state = result.state;
  }
  state.mode = "daily";
  return state;
...
export function bestDailyPitIndexes(state) {
  const actions = getLegalActions(state);
  const scored = actions.map((action) => ({ action, score: scoreTurn(state, action, state.currentPlayer) }));
  const best = Math.max(...scored.map((item) => item.score));
  return scored.filter((item) => item.score === best).map((item) => item.action.pitIndex);
}
