...
const WARDEN_TOKEN = "/assets/heritage-arcade/asalto/breakwater-warden-token.webp";
...
export default function AsaltoBoard({ roster, onBack }: { roster: ClanId[]; onBack: () => void }) {
  ...
  const rebelCount = game.pieces.filter((piece) => piece.side === "rebel").length;
  const capturedCount = 24 - rebelCount;
  const fortHeld = Array.from(ASALTO_FORT).filter((node) => occupied(game, node)?.side === "rebel").length;
  const push = (next: State) => { setHistory((past) => [...past.slice(-49), game]); setGame(next); };
  const reset = () => { soundManager.unlock(); soundManager.play("uiConfirm"); setHistory([]); setGame(createAsaltoState()); setTab("play"); };

  const act = (node: string) => {
    soundManager.unlock();
    if (game.winner || tab !== "play") return;
    if (game.phase === "deploy") {
      if (!ASALTO_FORT.has(node) || occupied(game, node)) { soundManager.play("invalidAction", { volume: .35 }); return; }
      ...
      soundManager.play("uiConfirm", { volume: .45 });
    ...
    if (piece?.side === game.turn && (!game.captureChain || piece.id === game.captureChain)) { soundManager.play("pieceSelect", { volume: .45 }); push({ ...game, selected: game.selected === piece.id ? null : piece.id, note: `${piece.side === "officer" ? "Warden" : "Rebel"} selected.` }); return; }
    if (!game.selected || !legal.includes(node)) { soundManager.play("invalidAction", { volume: .35 }); return; }
    ...
    if (capture) {
      soundManager.play("capture", { volume: .68 });
    ...
    } else soundManager.play(missedCapture ? "teamEliminated" : "pieceMove", { volume: .5 });
    const resolved = finishTurn(moved, variant, missedCapture ? "A required jump was declined; that Warden is huffed." : capture ? "A Warden captured a Rebel." : `${moving.side === "officer" ? "Warden" : "Rebel"} moved.`);
    if (resolved.winner) soundManager.play("victory", { volume: .65 });
    push(resolved);
  };
  ...
  return <main className="compact-screen asalto-screen">
    ...
    {tab === "play" && <section className="asalto-layout"><aside className="match-rail"><div className="compass-status"><span className="pulse-dot" style={{ background: CLANS[activeClan].color }} /><span>{game.winner ? "EXPEDITION ENDED" : game.captureChain ? "CAPTURE CHAIN" : game.phase === "deploy" ? "DEPLOYMENT" : "ACTIVE TURN"}</span></div><h2>{game.winner ? `${game.winner === "officer" ? CLANS[officerClan].name : CLANS[rebelClan].name} wins` : game.phase === "deploy" ? "Choose the Icegate" : `${game.turn === "officer" ? CLANS[officerClan].name : CLANS[rebelClan].name}'s turn`}</h2><p aria-live="polite">{game.note}</p><div className="match-player-stack"><article className={`match-player ${game.turn === "officer" ? "is-turn" : ""}`} style={{ "--clan": CLANS[officerClan].color } as React.CSSProperties}><img src={WARDEN_TOKEN} alt="" /><div><strong>Breakwater Wardens</strong><span>{CLANS[officerClan].name} · {game.pieces.filter((piece) => piece.side === "officer").length} remaining</span></div></article><article className={`match-player ${game.turn === "rebel" ? "is-turn" : ""}`} style={{ "--clan": CLANS[rebelClan].color } as React.CSSProperties}><img src={CLANS[rebelClan].portrait} alt="" /><div><strong>Icebound Rebels</strong><span>{CLANS[rebelClan].name} · {rebelCount} advancing</span></div></article></div><div className="asalto-siege-meter"><div><span>Icegate pressure</span><strong>{fortHeld}/9</strong></div><i><b style={{ width: `${fortHeld / 9 * 100}%` }} /></i></div><label className="asalto-variant"><span>Capture rule</span><select value={variant} disabled={game.action > 0} onChange={(event) => setVariant(event.target.value as Variant)}><option value="guided">Guided mandatory</option><option value="huffing">Historic huffing</option></select><small>{variant === "guided" ? "Required jumps are enforced and highlighted." : "Declining an available jump removes the Warden moved."}</small></label></aside>
      <div className="asalto-board-column"><div className="asalto-turn-ribbon" style={{ "--active-clan": CLANS[activeClan].color } as React.CSSProperties}><span>{game.turn === "officer" ? "◆" : "▲"}</span><strong>{game.phase === "deploy" ? "Place two Wardens inside the gold Icegate" : game.captureChain ? "Continue the highlighted jump" : `${game.turn === "officer" ? "Wardens" : "Rebels"} command the floe`}</strong><small>FREE PLAY · LOCAL TABLE</small></div><section className={`asalto-board ${game.captureChain ? "is-chain" : ""}`} aria-label="Asalto board, fortress at the top"><div className="asalto-fort-label">◆ ICEGATE</div><svg className="asalto-routes" viewBox="0 0 700 700" aria-hidden="true">{ASALTO_EDGES.map(([a, b]) => { const p = pointMap.get(a)!; const q = pointMap.get(b)!; return <line key={`${a}-${b}`} x1={50 + p.col * 100} y1={50 + p.row * 100} x2={50 + q.col * 100} y2={50 + q.row * 100} />; })}<path className="fort-outline" d="M225 18 H475 V275 H225 Z" /></svg><div className="asalto-stamp">33 POINTS · 9-POINT ICEGATE</div>{ASALTO_POINTS.map((point) => { const piece = occupied(game, point.id); const deploy = game.phase === "deploy" && ASALTO_FORT.has(point.id) && !piece; const mustCapture = piece && mandatoryOfficerIds.has(piece.id); const portrait = piece?.side === "officer" ? WARDEN_TOKEN : CLANS[rebelClan].portrait; return <button key={point.id} type="button" onClick={() => act(point.id)} className={`asalto-node ${ASALTO_FORT.has(point.id) ? "fort" : ""} ${legal.includes(point.id) ? "legal" : ""} ${game.selected === piece?.id ? "selected" : ""} ${mustCapture ? "must-capture" : ""}`} style={{ left: `${7.14 + point.col * 14.286}%`, top: `${7.14 + point.row * 14.286}%` }} aria-label={`${piece ? `${piece.side} piece` : "empty point"}${ASALTO_FORT.has(point.id) ? ", Icegate" : ""}${legal.includes(point.id) ? ", legal destination" : ""}`}>{(piece || deploy) && <span className={piece ? `asalto-piece ${piece.side}` : "deploy-marker"} style={piece ? { backgroundImage: `url(${portrait})`, "--piece": CLANS[piece.side === "officer" ? officerClan : rebelClan].color } as React.CSSProperties : undefined} />}</button>; })}{game.winner && <div className="asalto-victory"><small>THE BREAKWATER DECIDES</small><h2>{game.winner === "officer" ? "The Icegate holds" : "The fortress falls"}</h2><p>{game.note}</p><button onClick={reset}>Play again</button></div>}</section><div className="asalto-board-hud"><div><span>Captured Rebels</span><strong>{capturedCount}</strong></div><div className="asalto-capture-track" aria-hidden="true">{Array.from({ length: 16 }, (_, index) => <i key={index} className={index < capturedCount ? "filled" : ""} />)}</div><div><span>Turn</span><strong>{game.action}</strong></div></div></div>
