export const POLAR_TABLAN_RULESET = Object.freeze({
  gameId: "polar-tablan",
...
export const ROUTE_LENGTH = ROWS * COLS;

export function cellId(row, col) { return `t${row}-${col}`; }
export function parseCell(id) { const [, row, col] = /^t(\d+)-(\d+)$/.exec(id) || []; return { row: Number(row), col: Number(col) }; }

function rowCells(row, direction) {
  return Array.from({ length: COLS }, (_, index) => cellId(row, direction === "right" ? index : COLS - 1 - index));
}
...
export function opponentHomeRow(side) { return side === "aurora" ? 0 : 3; }
export function ownHomeRow(side) { return side === "aurora" ? 3 : 0; }

function createPieces(side) {
  return Array.from({ length: COLS }, (_, col) => ({
    id: `${side}-${col + 1}`,
    side,
    progress: side === "aurora" ? col : COLS - 1 - col,
    started: false,
    status: "active"
  }));
}

export function createPolarTablanState({ mode = "hotseat", starter = "aurora", seed = Date.now() } = {}) {
  const state = {
    gameId: POLAR_TABLAN_RULESET.gameId,
    rulesetVersion: POLAR_TABLAN_RULESET.rulesetVersion,
    mode,
...
export function createFinishRowDrill() {
  ...
  runner.status = "active"; runner.progress = 28; runner.started = true;
  const defender = state.pieces.ember[0];
  defender.status = "active"; defender.progress = 0; defender.started = false;
...
export function scoreSticks(faces) {
  const plainUp = faces.reduce((sum, face) => sum + Number(Boolean(face)), 0);
  return { plainUp, value: POLAR_TABLAN_RULESET.scoring[plainUp] ?? 0, scores: [0,1,4].includes(plainUp) };
}

export function getPiece(state, side, pieceId) { return (state.pieces[side] || []).find((piece) => piece.id === pieceId) || null; }
export function getPieceCell(piece) { return piece?.status === "active" || piece?.status === "locked" ? routeFor(piece.side)[piece.progress] || null : null; }
export function occupantAt(state, id, ignored = []) {
  const ignore = new Set(Array.isArray(ignored) ? ignored : [ignored]);
  for (const side of SIDES) for (const piece of state.pieces[side]) if (!ignore.has(piece.id) && getPieceCell(piece) === id) return piece;
  return null;
}

function canStart(piece, amount, rollValue, split) {
  if (piece.started) return true;
  return rollValue === 2 && amount === (split ? 1 : 2);
}
...
function simulateLeg(state, side, leg, rollValue, split) {
  ...
  if (occupant) {
    ...
    const capturable = row === 1 || row === 2 || row === opponentHomeRow(side);
    if (!capturable) return { error: "Enemy runners cannot be captured on your own home row." };
    occupant.status = "captured";
    occupant.progress = null;
...
function candidateAction(state, side, legs, split) {
  ...
  for (const leg of legs) {
    const result = simulateLeg(draft, side, leg, state.pendingRoll.value, split);
...
export function getLegalActions(state, side = state.currentPlayer) {
  ...
  for (const piece of state.pieces[side]) {
    const action = candidateAction(state, side, [{ pieceId: piece.id, amount: value }], false);
...
export function applyRoll(state, faces, side = state.currentPlayer, meta = {}) {
  ...
  const scored = scoreSticks(normalized);
  ...
  next.lastRoll = roll;
  next.castCount += 1;
  next.history.push({ type: "roll", turn: next.turn, side, roll: clone(roll) });
  if (!scored.value) {
    ...
    next.pendingRoll = { value: scored.value, plainUp: scored.plainUp, faces: normalized, splitValue: roll.splitValue, throwNumber: roll.throwNumber };
...
export function applyAction(state, action, side = state.currentPlayer) {
  ...
  if (legal.type === "forfeit-roll") {
    next.lastAction = { type: "forfeit-roll", side, value: legal.value };
  } else {
    const resolved = [];
    for (const leg of legal.legs) {
      const result = simulateLeg(next, side, leg, next.pendingRoll.value, legal.split);
      if (result.error) return { state, error: result.error };
      if (result.capturedPieceId) next.captures[side] += 1;
      resolved.push(result);
    }
    next.lastAction = { type: "move", side, split: legal.split, value: legal.value, legs: resolved };
  }
  next.history.push({ type: "action", turn: next.turn, side, action: clone(next.lastAction) });
  next.pendingRoll = null;
  next.awaiting = "roll";
  if (raceComplete(next, side)) finalizeRace(next, side);
  else if (next.bonusRolls > 0) next.bonusRolls -= 1;
  else { next.currentPlayer = otherSide(side); next.turn += 1; }
  assertStateInvariant(next);
  return { state: next, error: null };
}

function raceComplete(state, side) {
  const survivors = state.pieces[side].filter((piece) => piece.status !== "captured");
  return survivors.every((piece) => piece.status === "locked");
}

function finalizeRace(state, finisher) {
  const scores = Object.fromEntries(SIDES.map((side) => [side, state.pieces[side].filter((piece) => piece.status === "locked").length]));
  state.scores = scores;
  state.winner = scores.aurora === scores.ember ? finisher : scores.aurora > scores.ember ? "aurora" : "ember";
  state.winReason = scores.aurora === scores.ember ? "first-finisher-tiebreak" : "finish-row-majority";
  state.awaiting = "finished";
  state.pendingRoll = null;
  state.bonusRolls = 0;
}

export function rollLocalSticks(state, side = state.currentPlayer) {
...
export function getPlayerSummary(state, side) {
  ...
  return { active: pieces.filter((piece) => piece.status === "active").length, locked: pieces.filter((piece) => piece.status === "locked").length, captured: pieces.filter((piece) => piece.status === "captured").length, captures: state.captures[side], progress: pieces.reduce((sum, piece) => sum + (Number(piece.progress) || 0), 0) };
}

export function describeTurn(state) {
  if (state.winner) return resultTitle(state);
  return state.awaiting === "roll" ? `${sideName(state.currentPlayer)}: cast four sticks.` : `${sideName(state.currentPlayer)}: use ${state.pendingRoll.value} in full or split it.`;
}
export function resultTitle(state) { return state.winner ? `${sideName(state.winner)} wins the finish row` : "Polar Tablan in progress"; }
export function resultDetail(state) { return state.scores ? `Final locked score: Aurora ${state.scores.aurora}, Ember ${state.scores.ember}.` : "Race every surviving runner into the rival home row."; }
export function actionSummary(action) {
  if (!action) return "";
  if (action.type === "forfeit-roll") return `${sideName(action.side)} had no legal way to use ${action.value}.`;
  const captures = action.legs.filter((leg) => leg.capturedPieceId).length;
  const locks = action.legs.filter((leg) => leg.locked).length;
  return `${sideName(action.side)} used ${action.value}${action.split ? " as a split move" : ""}${captures ? `, captured ${captures}` : ""}${locks ? `, and locked ${locks} runner${locks === 1 ? "" : "s"} into the finish row` : ""}.`;
}

function actionKey(action) {
  if (!action) return "";
  if (action.type === "forfeit-roll") return `forfeit:${action.value}`;
  return `${action.type}:${action.split ? 1 : 0}:${(action.legs || []).map((leg) => `${leg.pieceId}@${leg.amount}`).join("|")}`;
}

export function assertStateInvariant(state) {
  const all = SIDES.flatMap((side) => state.pieces[side] || []);
  if (all.length !== 24) throw new Error("Polar Tablan must keep twelve records per side.");
  const occupied = new Set();
  for (const piece of all) {
    if (!["active","locked","captured"].includes(piece.status)) throw new Error("Unknown Polar Tablan runner status.");
    if (piece.status === "captured") { if (piece.progress !== null) throw new Error("Captured runner cannot retain route progress."); continue; }
    if (!Number.isInteger(piece.progress) || piece.progress < 0 || piece.progress >= ROUTE_LENGTH) throw new Error("Runner has invalid route progress.");
    const id = getPieceCell(piece);
    if (occupied.has(id)) throw new Error(`Two runners occupy ${id}.`);
    occupied.add(id);
    if (piece.status === "locked" && parseCell(id).row !== opponentHomeRow(piece.side)) throw new Error("Locked runner is outside the rival home row.");
  }
  return true;
}
