export const CROWN_RUN_RULESET = Object.freeze({
  gameId: "crown-run",
...
export const CENTER_PROGRESS = CROWN_RUN_RULESET.trackLength;
export const FINISHED_PROGRESS = CROWN_RUN_RULESET.trackLength + 1;
export const OPPONENT_HOME_START = CROWN_RUN_RULESET.trackLength - CROWN_RUN_RULESET.homeRowLength;
...
export const SPACES = Object.freeze(makeSpaces());
export const SPACE_BY_ID = Object.freeze(Object.fromEntries(SPACES.map((space) => [space.id, space])));
export const SAFE_SPACES = Object.freeze(new Set(SPACES.filter((space) => space.safe).map((space) => space.id)));
export const TRACK_LINES = Object.freeze(SPACES.slice(0, -1).map((space, index) => [space.id, SPACES[index + 1].id]));
...
function createPieces(side) {
  return [
    ...
    ...Array.from({ length: CROWN_RUN_RULESET.standardPiecesPerSide }, (_, index) => ({ id: `${side}-kaangi-${index + 1}`, side, kind: "standard", status: "home", progress: -1 }))
  ];
}

export function createCrownRunState({ mode = "hotseat", starter = "aurora", seed = Date.now() } = {}) {
  const state = {
...
export function createCrownCollapseDrill() {
  ...
  state.throwPool = [{ id: "drill-3", value: 3, mouthsUp: 3, faces: [1, 1, 1, 0, 0], bonus: false }];
...
export function getPiecesAtSpace(state, spaceId) { return SIDES.flatMap((side) => (state.pieces[side] || []).filter((piece) => piece.status === "track" && getPieceSpaceId(piece) === spaceId)); }
export function getOpponentPiecesAtSpace(state, side, spaceId) { return getPiecesAtSpace(state, spaceId).filter((piece) => piece.side !== side); }
...
function canLand(state, side, spaceId) { return !(getOpponentPiecesAtSpace(state, side, spaceId).length && isSafeSpace(spaceId)); }
...
function actionForMove(state, piece, throwItem) {
  ...
  return { type: "move", pieceId: piece.id, throwId: throwItem.id, value, targetProgress, targetSpace, entersCenter: targetProgress === CROWN_RUN_RULESET.trackLength - 1, capturedPieceId: captured?.id || null };
}
...
export function getLegalActions(state, side = state.currentPlayer) {
  ...
  for (const throwItem of state.throwPool) {
    ...
    if (value === 1 && !waiting.length) centerPieces(state, side).forEach((piece) => actions.push({ type: "exit", pieceId: piece.id, throwId: throwItem.id, value }));
...
export function applyAction(state, action, side = state.currentPlayer) {
  ...
  else if (legal.type === "exit") { piece.status = "finished"; piece.progress = FINISHED_PROGRESS; }
  else if (legal.type === "move") {
    if (legal.entersCenter) { piece.status = "center"; piece.progress = CENTER_PROGRESS; }
  ...
  next.lastMove = { type: legal.type, side, pieceId: piece.id, pieceKind: piece.kind, throwId: legal.throwId, value: legal.value, fromSpace, targetSpace: legal.targetSpace || getPieceSpaceId(piece), entered: legal.type === "enter", exited: legal.type === "exit", enteredCenter: legal.type === "move" && Boolean(legal.entersCenter), capturedPieceId: capturedPiece?.id || null, capturedKind: capturedPiece?.kind || null, resetCount };
  next.history.push({ type: "allocation", turn: next.turn, side, move: cloneState(next.lastMove) });

  if (capturedPiece) next.awaiting = "capture-roll";
  else if (next.pieces[side].every((candidate) => candidate.status === "finished")) {
    next.winner = side; next.winReason = "all-nine-exited"; next.awaiting = "finished"; next.throwPool = []; next.rollSequence = [];
  } else if (!next.throwPool.length || !getLegalActions(next, side).length) finishTurn(next, next.throwPool.length ? "unused-throws-no-legal-application" : null);
...
function capturePiece(state, attacker, capturedPiece) {
  ...
  if (capturedPiece.kind === "king") state.lastReset = { victimSide, capturedBy: attacker.kind, resetPieceIds: resetPieces.map((piece) => piece.id), includedFinishedPieces: attacker.kind === "king" };
  return { capturedPiece, resetCount: resetPieces.length };
}

export function validateRollSequence(state, sequence, side = state.currentPlayer) {
  if (!state) return { valid: false, reason: "Missing Crown Run state." };
  if (state.winner) return { valid: false, reason: "The Crown Run match has already ended." };
  if (side !== state.currentPlayer) return { valid: false, reason: "It is not this caravan's turn." };
  ...
  for (const roll of sequence) if (!Array.isArray(roll.faces) || roll.faces.length !== 5 || roll.faces.some((face) => ![0, 1, false, true].includes(face))) return { valid: false, reason: "Each Dadu cast must contain five binary cowries." };
  return { valid: true };
}
...
export function getSideSummary(state, side) {
  const pieces = state.pieces[side] || [];
  return { home: pieces.filter((piece) => piece.status === "home").length, track: pieces.filter((piece) => piece.status === "track").length, center: pieces.filter((piece) => piece.status === "center").length, finished: pieces.filter((piece) => piece.status === "finished").length, captures: Number(state.captures[side] || 0), kingResets: Number(state.kingResets[side] || 0), captureLicense: Boolean(state.captureLicense[side]) };
}
...
export function resultTitle(state) { return !state.winner ? "The crown race continues" : state.winner === "aurora" ? "Aurora Court secures the crown" : "Ember Court secures the crown"; }
export function resultDetail(state) { return state.winReason === "all-nine-exited" ? "All eight kaangi and the nakta have crossed the opposing home row and exited." : "The Crown Run match is complete."; }
export function actionKey(action) { return action ? `${action.type}:${action.pieceId || ""}:${action.throwId || ""}` : ""; }
export function actionSummary(move) {
  if (!move) return "";
  const side = move.side === "aurora" ? "Aurora" : "Ember";
  const piece = move.pieceKind === "king" ? "nakta" : "kaangi";
  if (move.capturedKind === "king") return `${side} ${piece} captured the rival nakta and reset ${move.resetCount} piece${move.resetCount === 1 ? "" : "s"}. Cast the capture throw.`;
  if (move.capturedPieceId) return `${side} ${piece} captured one rival piece. Cast the capture throw.`;
  if (move.entered) return `${side} entered a ${piece} with da.`;
  if (move.exited) return `${side} exited a ${piece} from the central quadrant.`;
  if (move.enteredCenter) return `${side} moved a ${piece} into the central quadrant.`;
  return `${side} moved a ${piece} by ${move.value}.`;
}
