export const SIXTEEN_ICE_WARRIORS_RULESET = Object.freeze({
...
export function createSixteenIceWarriorsState({ mode = "hotseat", starter = "aurora" } = {}) {
...
export function createCaptureDrillState() {
...
export function getLegalActions(state, side = state.currentPlayer) {
  ...
  if (state.chainFrom) return [...getCaptureActions(state, side, state.chainFrom), { type: "end-chain", from: state.chainFrom }];
  const actions = [];
...
export function actionSummary(action) {
  if (!action) return "";
  const label = action.player === "aurora" ? "Aurora" : "Ember";
  if (action.type === "capture") return `${label} chopped a soldier and landed on ${action.to}.`;
  if (action.type === "end-chain") return `${label} ended the capture chain.`;
