export const FOUR_WING_ICE_HUNT_RULESET = Object.freeze({
...
export function createFourWingIceHuntState({ mode = "hotseat" } = {}) {
  const state = {
...
export function getLegalActions(state, player = state.currentPlayer) {
  ...
  if (phase === "leopard-placement") {
    return emptyNodes(state).map((nodeId) => ({ type: "place", nodeId }));
  }
  if (phase === "cattle-deployment") {
    return emptyNodes(state)
      .filter((nodeId) => state.cattlePlaced > 0 || firstCattleIsSafe(state, nodeId))
      .map((nodeId) => ({ type: "place", nodeId }));
  }
  if (player === "leopards") return getLeopardActions(state);
  return getCattleMoves(state);
}
...
export function getLeopardActions(state) {
  ...
  occupiedNodes(state, "leopards").forEach((from) => {
    ...
    JUMP_PATHS.filter((path) => path.from === from)
...
export function getCattleMoves(state) {
  const actions = [];
  occupiedNodes(state, "cattle").forEach((from) => {
    (ADJACENCY[from] || []).filter((to) => !state.board[to]).forEach((to) => actions.push({ type: "move", from, to }));
  });
  return actions;
}
...
export function getCounts(state) {
  return {
    ...
    cattleInHand: FOUR_WING_ICE_HUNT_RULESET.cattle - state.cattlePlaced,
...
export function describeTurn(state) {
  ...
  if (phase === "leopard-placement") return state.leopardsPlaced === 0 ? "Place the first snow leopard." : "Place the second snow leopard.";
  if (phase === "cattle-deployment") return state.cattlePlaced === 0 ? "Place the first cattle piece where it cannot be captured immediately." : "Deploy one cattle piece.";
  return state.currentPlayer === "leopards" ? "Move or jump-capture with one leopard." : "Move one cattle piece to tighten the trap.";
}
...
export function resultDetail(state) {
  return {
    "all-cattle-captured": "All twenty-four cattle pieces were captured.",
    "leopards-imprisoned": "Both leopards have been immobilised.",
...
function deploymentComplete(state) {
  return state.leopardsPlaced === FOUR_WING_ICE_HUNT_RULESET.leopards && state.cattlePlaced === FOUR_WING_ICE_HUNT_RULESET.cattle;
}
...
function positionSignature(state) {
  return `${NODES.map((node) => state.board[node.id] === "leopards" ? "L" : state.board[node.id] === "cattle" ? "C" : "-").join("")}|${state.currentPlayer}|${state.leopardsPlaced}|${state.cattlePlaced}`;
}
