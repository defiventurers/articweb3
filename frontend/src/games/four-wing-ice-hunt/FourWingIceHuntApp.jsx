...
export function FourWingIceHuntApp({ onExitToLibrary, profile, onProfileChange }) {
  ...
  if (screen === "cover") return <section className="fwh-cover" aria-label="Four-Wing Ice Hunt cover"><button className="fwh-back-pill" onClick={onExitToLibrary}>← All Games</button><div className="fwh-cover-compass" aria-hidden="true"><span /><span /><span /><span /><i>✦</i></div><div className="fwh-cover-copy"><p>FOUR-WING HUNT · STANDARD RULESET</p><h1>FOUR-WING<br />ICE HUNT</h1><span>Two snow leopards. Twenty-four penguin colonisers. Four frozen wings.</span><button onClick={() => setScreen("menu")}>Enter the hunt</button></div></section>;
  if (screen === "menu") return <section className="fwh-menu" aria-label="Four-Wing Ice Hunt menu"><button className="fwh-back-pill" onClick={() => setScreen("cover")}>← Cover</button><article className="fwh-menu-card"><p className="fwh-eyebrow">FOUR-WING HUNT · STANDARD RULESET</p><h1>Four-Wing Ice Hunt</h1><p>Deploy twenty-four penguin colonisers to imprison two mobile snow leopards. Nine penguins begin in the central reference formation; the remaining fifteen arrive between leopard turns.</p><div className="fwh-menu-actions"><button className="primary" onClick={() => setScreen("online")}>Online Multiplayer</button><button onClick={() => startGame("practice-leopards")}>Practice as Snow Leopards</button><button onClick={() => startGame("practice-cattle")}>Practice as the Colony</button><button onClick={() => startGame("hotseat")}>Local Two Player</button><button onClick={() => setScreen("rules")}>How to Play</button></div><div className="fwh-source-note">Reference opening: two leopards begin outside opposite corners; nine penguins hold the central three-by-three pattern; fifteen reserves deploy one at a time between leopard turns. Leopards win at twelve captures. The colony wins by immobilising both hunters.</div></article></section>;
