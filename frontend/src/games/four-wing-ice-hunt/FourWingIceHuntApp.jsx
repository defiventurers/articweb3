...
export function FourWingIceHuntApp({ onExitToLibrary, profile, onProfileChange }) {
  ...
  if (screen === "menu") return <section className="fwh-menu" aria-label="Four-Wing Ice Hunt menu"><button className="fwh-back-pill" onClick={() => setScreen("cover")}>← Cover</button><article className="fwh-menu-card"><p className="fwh-eyebrow">FOUR-WING HUNT · STANDARD RULESET</p><h1>Four-Wing Ice Hunt</h1><p>Deploy twenty-four penguin colonisers to imprison two mobile snow leopards. Eight penguins begin around an open central court; the remaining sixteen arrive between leopard turns.</p><div className="fwh-menu-actions"><button className="primary" onClick={() => setScreen("online")}>Online Multiplayer</button><button onClick={() => startGame("practice-leopards")}>Practice as Snow Leopards</button><button onClick={() => startGame("practice-cattle")}>Practice as the Colony</button><button onClick={() => startGame("hotseat")}>Local Two Player</button><button onClick={() => setScreen("rules")}>How to Play</button></div><div className="fwh-source-note">Reference opening: two leopards begin outside opposite corners; eight penguins surround an open centre court; sixteen reserves deploy one at a time between leopard turns. Leopards win at twelve captures. The colony wins by immobilising both hunters.</div></article></section>;
