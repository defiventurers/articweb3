...
export function KhasiFishflowBoard({ state, onPit, interactivePlayer = state.currentPlayer, interactive = true }) {
  ...
  return (
    <div className="fishflow-board khasi-board" role="grid" aria-label="Khasi Fishflow Mawkar Katiya board">
      ...
      <div className="fishflow-pit-field">
        ...
        <div className="fishflow-row coral" role="row">
          {[...state.rows.coral].map((count, displayIndex) => {
            ...
            return <Pit key={`coral-${pitIndex}`} player="coral" pitIndex={pitIndex} count={count} active={pitIndex < state.activePits.coral} legal={state.currentPlayer === "coral" && legalIndexes.has(pitIndex)} onPit={onPit} />;
          })}
        </div>
        <div className="fishflow-flow-line" aria-hidden="true"><span>→ CLOCKWISE CURRENT →</span></div>
        <div className="fishflow-row blue" role="row">
          {state.rows.blue.map((count, pitIndex) => <Pit key={`blue-${pitIndex}`} player="blue" pitIndex={pitIndex} count={count} active={pitIndex < state.activePits.blue} legal={state.currentPlayer === "blue" && legalIndexes.has(pitIndex)} onPit={onPit} />)}
        </div>
...
function Store({ player, count }) {
  return <aside className={`fishflow-store ${player}`} aria-label={`${player} fish store with ${count} fish`}><span aria-hidden="true">◒</span><strong>{count}</strong><small>ICE STORE</small></aside>;
}
