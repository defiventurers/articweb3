export const SKY_TEMPLE_RUN_RULESET = Object.freeze({
...
function expandRoute(anchors) {
  ...
  for (let index = 0; index < anchors.length - 1; index += 1) {
    ...
    route.push(...edgeMidIds(from, to), to);
  }
  return route;
}
...
export const GATE_PROGRESS = Object.freeze({
  aurora: ROUTES.aurora.indexOf("k"),
  ember: ROUTES.ember.indexOf("e")
});
export const FINISH_PROGRESS = ROUTES.aurora.length - 1;
export const SAFE_SPACES = Object.freeze(new Set(Object.keys(ANCHOR_COORDS)));
...
function createSpaces() {
  ...
  for (const anchors of Object.values(ANCHOR_ROUTES)) {
    for (let index = 0; index < anchors.length - 1; index += 1) {
      ...
      seen.add(key);
      const [ax, ay] = ANCHOR_COORDS[key.split("-")[0]];
      const [bx, by] = ANCHOR_COORDS[key.split("-")[1]];
      for (let step = 1; step <= INTERMEDIATE_PER_EDGE; step += 1) {
        ...
        spaces.push({
          ...
          x: ax + (bx - ax) * ratio,
          y: ay + (by - ay) * ratio,
          label: `${key.toUpperCase()} corridor ${step}`,
          kind: "corridor"
        });
      }
    }
...
export const SPACES = Object.freeze(createSpaces());
export const SPACE_BY_ID = Object.freeze(Object.fromEntries(SPACES.map((space) => [space.id, space])));
export const TRACK_LINES = Object.freeze(uniqueEdges().map(([from, to]) => [from, to]));
...
function createPieces(side) {
  return Array.from({ length: SKY_TEMPLE_RUN_RULESET.piecesPerPlayer }, (_, index) => ({
    id: `${side}-${index + 1}`,
    side,
    status: "home",
    progress: -1
...
export function createSkyTempleRunState({ mode = "hotseat", starter = "aurora", seed = Date.now() } = {}) {
  return {
...
export function createTempleGateDrill() {
...
export function getLegalActions(state, side = state.currentPlayer) {
  ...
  if (SKY_TEMPLE_RUN_RULESET.entryValues.includes(value) && canLand(state, side, route[0])) {
    for (const piece of state.pieces[side]) {
      if (piece.status === "home") actions.push({ type: "enter", pieceId: piece.id, value, targetProgress: 0, targetSpace: route[0] });
    }
  }
  ...
  for (const piece of state.pieces[side]) {
    ...
    actions.push({
      ...
      captures: occupant && occupant.side !== side && !SAFE_SPACES.has(targetSpace) ? occupant.id : null
    });
  }
  return actions;
}
...
export function validateRoll(state, faces, side = state.currentPlayer) {
  ...
  if (!Array.isArray(faces) || faces.length !== SKY_TEMPLE_RUN_RULESET.cowries || faces.some((face) => ![0, 1, false, true].includes(face))) {
    return { valid: false, reason: "A Vimanam cast must contain six open-or-closed cowries." };
  }
  return { valid: true };
}
...
export function applyRoll(state, faces, side = state.currentPlayer, meta = {}) {
  ...
  const roll = {
    ...
    mouthsUp,
    value,
    bonus: SKY_TEMPLE_RUN_RULESET.bonusValues.includes(value),
    proofHash: meta.proofHash || null,
    nonce: meta.nonce || null,
    throwNumber: next.castCount + 1
  };
  next.roll = roll;
...
export function applyAction(state, action, side = state.currentPlayer) {
  ...
  next.lastMove = {
    ...
    unlockedGate: !hadLicense && Boolean(capturedPiece),
  ...
  if (next.pieces[side].every((candidate) => candidate.status === "finished")) {
    ...
    finishThrow(next, Boolean(capturedPiece));
  }
  assertStateInvariant(next);
  return { state: next, error: null };
}
...
export function getPlayerSummary(state, side) {
  ...
  return {
    ...
    gateUnlocked: Boolean(state.captureLicense[side]),
...
export function describeTurn(state) {
  ...
  return `${label}: move one pilgrim by ${state.roll?.value || 0}.`;
}
...
export function resultDetail(state) {
  return state.winReason === "all-six-finished"
    ? "All six pilgrims completed the Vimanam route with an exact final cast."
    : "The Sky Temple Run is complete.";
}
