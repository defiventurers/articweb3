import { appConfig, getClosedBetaAccessIssue, getHighStakesConfigIssue } from "../../config/chain.js";
...
export function HighStakesGate({ children, onBack }) {
  ...
  const { address, isConnected, isWrongChain, connectedChainId, expectedChainId, expectedNetworkName } = useChainGuard();
  ...
  if (!blockReason) blockReason = getClosedBetaAccessIssue(address);
      ...
      <div className="highstakes-shell">
        <div className="highstakes-config-panel" role="status">
          ...
          <dl className="highstakes-config-list">
            ...
            <div><dt>Closed Beta gate</dt><dd>{String(appConfig.closedBeta.highStakes)}</dd></div>
            <div><dt>Allowed wallet count</dt><dd>{appConfig.closedBeta.allowedWallets.length}</dd></div>
