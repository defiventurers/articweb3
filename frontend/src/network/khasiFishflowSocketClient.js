...
export const createKhasiFishflowRoom = async ({ visibility = "public", side = "aurora", profile }) => (await request("kf_room_create", { visibility, side, wallet: profile.wallet })).room;
...
export const getKhasiFishflowLegalActions = async ({ roomCode, profile }) => (await request("kf_legal_actions", { roomCode, wallet: profile.wallet })).actions || [];
